...
class World():
	...
	def plusOneE(self):
		if(self.nivelEsquerdo < 2):
			self.nivelEsquerdo=self.nivelEsquerdo + 1
	...
	def plusOneD(self):
		if(self.nivelDireito <2):
			self.nivelDireito=self.nivelDireito + 1

	def minusOneD(self):
		if(self.nivelDireito >1):
			self.nivelDireito = self.nivelDireito - 1
	...
	def __init__(self, world):
		...
		self.world = world
		self.world.accept("arrow_up",self.plusOneE)
		self.world.accept("arrow_down",self.minusOneE)
		self.world.accept("arrow_left",self.minusOneD)
		self.world.accept("arrow_right",self.plusOneD)
		print "world started"
	...
	def moveReference(self,task):
		...
		y = int(interp(float(y),[-400,600],[10,195]))
	...
	def createTarget(self,modelo="misc/models/star"):
		...
		if not self.world.side:
			...
			'''
			if(self.nivelEsquerdo == 1):
				self.target.setPos(random.randint(0,150),random.choice([440,540]),random.randint(90,130))
			elif(self.nivelEsquerdo == 2):
				self.target.setPos(random.randint(0,200),random.choice([450,640]),random.randint(110,170))
			...
			'''
			if(self.nivelEsquerdo == 1):
				self.target.setPos(random.randint(-150,0),random.choice([440,540]),random.randint(90,130))
			elif(self.nivelEsquerdo == 2):
				self.target.setPos(random.randint(-200,0),random.choice([450,640]),random.randint(110,170))
